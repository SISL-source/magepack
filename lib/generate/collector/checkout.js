...
const checkout = async (
    ...
    const response = await page.goto(productUrl, { waitUntil: 'networkidle0' });
    if (!response || !response.ok()) {
        throw new Error(
            `Page "${productUrl}" returned HTTP ${response ? response.status() : 'no response'} - cannot collect modules.`
        );
    }
    ...
    await page.evaluate(() => {
        ...
        Array.from(swatches).forEach((swatch) => {
            ...
            const swatchInput = swatch.querySelector('.swatch-input');
            if (!swatchOption || !swatchInput) {
                return;
            }
            swatchInput.value =
